...
const FENCE_LANGS = new Set(['astro', 'ts', 'js', 'mjs', 'tsx', 'jsx']);
...
// ts/js/mjs/tsx/jsx: `src/...` OR the one repo-root exception `astro.config.mjs`.
// tsx/jsx added for framework-island fences (same path-comment rule as ts) —
// islands module needs this to collect a Preact/React .tsx component.
const TSJSMJS_TOKEN = String.raw`src\/[\w@.\-[\]()/]+\.(?:ts|js|mjs|tsx|jsx)|astro\.config\.mjs`;
...
//
// preact(): the course site's own framework, needed so a lesson's .tsx
// island fence (e.g. islands/framework-integrations) type-checks and
// builds — installed into tools/probe alongside @astrojs/node, see
// ensureProbe's dev-dependency install below.
...
function rewriteSpecifiers(body, fenceSrcPath, ns, owners, ownKeys) {
  ...
  return body.replace(
    /\b(from|import|require)(\s*\(?\s*)(['"])(\.\.?\/[^'"]*\.(?:astro|ts|js|mjs|tsx|jsx)|@\/[^'"]*\.(?:astro|ts|js|mjs|tsx|jsx))\3/g,
    (whole, kw, ws, q, spec) => {
...
// Fixed superset of the collections the lessons reference BY NAME
// (getCollection('blog'), reference('authors'), getLiveEntry('products')).
// In check mode every lesson's own content.config.ts / live.config.ts is
// namespaced away under src/lessons/<ns>/, so without these two real files
// `astro sync` emits no collection types and every getCollection('blog')
// call fails with "'blog' does not satisfy the constraint 'never'".
// Written on every run (safe to regenerate); --build backs them up and
// swaps a lesson's own config in, then restores these.
// ponytail: hand-maintained superset — add a field here when a new lesson
// reads `entry.data.<field>` on one of these collections.
const CONTENT_CONFIG_SRC = `// Generated by tools/verify-snippets.mjs — safe to regenerate, do not hand-edit.
...
const LIVE_CONFIG_SRC = `// Generated by tools/verify-snippets.mjs — safe to regenerate, do not hand-edit.
import { defineLiveCollection } from 'astro:content';
import type { LiveLoader } from 'astro/loaders';

interface Product { id: string; name: string; price: number }

const loader: LiveLoader<Product> = {
  name: 'probe-products',
  loadCollection: async () => ({ entries: [] }),
  loadEntry: async () => undefined,
};

export const collections = { products: defineLiveCollection({ loader }) };
`;

function writeProbeCollections() {
  for (const d of ['src/data/authors', 'src/data/blog']) mkdirSync(path.join(PROBE_DIR, d), { recursive: true });
  writeFileSync(path.join(PROBE_DIR, 'src/content.config.ts'), CONTENT_CONFIG_SRC);
  writeFileSync(path.join(PROBE_DIR, 'src/live.config.ts'), LIVE_CONFIG_SRC);
}
...
function patchTsconfig() {
  ...
  // jsx/jsxImportSource: the base astro tsconfig only sets `jsx: "preserve"`
  // (no import source), which doesn't type-check a .tsx island fence's JSX
  // against any runtime's types. Match the real course site's own
  // tsconfig.json (react-jsx / preact) so a Preact island fence type-checks.
  tsconfig.compilerOptions ??= {};
  if (tsconfig.compilerOptions.jsx !== 'react-jsx') {
    tsconfig.compilerOptions.jsx = 'react-jsx';
    changed = true;
  }
  if (tsconfig.compilerOptions.jsxImportSource !== 'preact') {
