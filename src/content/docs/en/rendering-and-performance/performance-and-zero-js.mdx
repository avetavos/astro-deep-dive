...
## The build itself got faster (Astro 7)

Runtime performance is what your users feel; build performance is what *you* feel every deploy. Astro 7 moved a lot of the toolchain to Rust: the Astro compiler, Markdown via Sätteri, and **Vite 8**, whose default bundler is now **Rolldown** (10–30× faster than Rollup while keeping the plugin API). A new **queued rendering engine** renders pages roughly 2.4× faster during the build. In Astro's own benchmarks builds are 15–61% faster, and some sites build twice as fast — with no changes to your code (source: [Astro 7.0 release notes](https://astro.build/blog/astro-7/)).
