...
**dynamic route** คือไฟล์ที่มีส่วนในวงเล็บ (`[slug].astro` หรือ `[...path]` สำหรับ rest param) ใน static build ไฟล์นี้ต้อง export **`getStaticPaths()`** ที่คืน `{ params, props }` ต่อหน้า หนึ่งไฟล์ HTML ต่อหนึ่ง entry ของ collection และ `paginate()` หั่น list ยาวเป็นหน้าแบบมีเลข สำหรับ URL ที่ระบุล่วงหน้าไม่ได้ ให้ตั้ง **`prerender = false`** ข้าม `getStaticPaths` แล้วอ่าน **`Astro.params`** สดต่อ request
</Callout>
