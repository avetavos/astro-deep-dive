...
description: 'scoped vs global CSS, ส่งค่าด้วย define:vars และ script tag ธรรมดาทำงานยังไงจริง ๆ — script รันบน browser แต่ไม่ใช่ island'
sidebar:
  order: 3
---
...
`<style>` ของ component เป็น **scoped โดย default**; opt out ด้วย `<style is:global>` หรือ selector `:global()` และส่ง props เข้า CSS ด้วย `define:vars` ส่วน `<script>` **รันบน browser** และถูก bundle/process โดย default (`is:inline` opt out) — แต่ **ไม่ใช่ island**: script แก้ DOM ที่มีอยู่โดยไม่มีการ hydrate component ถ้าอยากได้ UI component ที่มี state และ hydrate คุณต้องใช้ island (โมดูลถัดไป)
</Callout>
