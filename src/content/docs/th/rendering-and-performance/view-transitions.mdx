...
ใน Astro รุ่นเก่า component นี้ชื่อ `<ViewTransitions />` Astro ปัจจุบันเปลี่ยนชื่อเป็น **`<ClientRouter />`** (import จาก `astro:transitions`) เพื่อสื่อความหมายให้ชัดขึ้นว่าทำอะไร — เปิด client-side routing ถ้าเจอ `<ViewTransitions />` ใน tutorial เก่า ให้แทนที่ด้วย `<ClientRouter />` ในใจ
</Callout>
