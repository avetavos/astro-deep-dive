...
โปรเจกต์ Astro route ด้วยไฟล์ใน **`src/pages/`** และ serve `public/` ตรง ๆ ไฟล์ **`.astro`** มีสองส่วนแบ่งด้วย `---`: **component script** (frontmatter) ที่รันบน *server* — imports, top-level `await` ดึงข้อมูล, props, logic ซึ่งไม่มีอะไรถูกส่งไป browser — และ **template** (HTML บวก `{ }` expression) ที่กลายเป็น HTML ที่ render แล้ว เป็น superset ของ HTML ดังนั้น HTML ธรรมดาก็ทำงานได้เลย
</Callout>
