...
`server:defer` เปลี่ยน `.astro` component เป็น **server island**: เปลือกของหน้า render และออกทันที (จึง cache ได้) ส่วน component ที่ defer ไว้ render **บน server บน request ของตัวเอง** แล้ว HTML ของตัวเองถูกสลับเข้ามาแทน placeholder `slot="fallback"` เมื่อพร้อม นี่คือคำตอบของ "หน้านี้ cache ได้ ยกเว้นส่วนที่ personalize หรือช้าอยู่ส่วนหนึ่ง" — cache เปลือก defer ส่วน dynamic ตัว server island render บน server จึง **ต้องมี adapter** และเป็นเรื่องการ defer output จาก server ไม่ใช่ client interactivity
</Callout>
