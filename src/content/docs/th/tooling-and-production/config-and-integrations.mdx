...
integration ทางการที่พบบ่อย (`@astrojs/*`): `react` / `vue` / `svelte` / `solid` / `preact` (UI framework island), `mdx` (component ใน Markdown), `sitemap`, `rss` และ adapter ของแต่ละ platform (`cloudflare`, `node`, `vercel`, `netlify`)
